...
shards_dir=/data/duhu/shards/multi_dialect
...
bpemodel=$(dirname $dict)/MDB_${model_type}_${vocab_size} # multi-dialect bpe model => mdb
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  projs=`ls data | grep -v "train" | grep -v "dev"`
  train_folders=""
  test_folders=""
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    ./tools/spm_train --input=$dict_dir/input.txt --vocab_size=${vocab_size} --model_type=${model_type} --model_prefix=${bpemodel} --input_sentence_size=100000000  --user_defined_symbols=$user_defined_symbols
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for x in ${train_set}; do
    dst=$shards_dir/$x
    mkdir -p $dst
    tools/make_shard_list.py --resample 16000 --num_utts_per_shard 10000 \
      --num_threads 24 --segments data/$x/segments \
  ...
  for x in $test_sets; do
    ...
    mkdir -p $dst
    tools/make_shard_list.py --resample 16000 --num_utts_per_shard 10000 \
      --num_threads 24 --segments data/$x/test/segments \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  for testset in ${test_sets}; do
  {
    ...
    python wenet/bin/recognize.py --gpu ${device_id} \
      ...
      --data_type "shard" \
      --test_data data/$testset/test/data.list \
