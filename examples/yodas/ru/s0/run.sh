...
wave_data=data
data_type=shard
shards_dir=/data/duhu/shards/yodas
...
train_set=raw_clean
dev_set=dev
recog_set="cv17 fleurs dataocean"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  cat $wave_data/lang_char/${train_set}_${bpemode}${nbpe}.vocab | awk -F"\t" '{print $1}' | grep -v "<unk>" | grep -v "<s>" | grep -v "</s>" | sort | uniq | awk '{print $0 " " NR+2}' >> ${dict}
