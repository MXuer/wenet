...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  # Data preparation
  python local/aishell_data_prep.py --aishell-dir ${data} || exit 1;
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  echo "<sos/eos> 2" >> ${dict}
  echo "<scd> 3" >> ${dict}
  tools/text2token.py -s 1 -n 1 data/train/text_pure | cut -f 2- -d" " \
