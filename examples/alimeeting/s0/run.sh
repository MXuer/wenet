...
decode_checkpoint=exp/conformer_u2pp/40.pt
average_num=30
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  # Data preparation
  python local/alimeeting_data_prep.py -d $data
fi
  ...
  for x in dev test ${train_set}; do
    if [ $data_type == "shard" ]; then
      tools/make_shard_list.py --num_utts_per_shard $num_utts_per_shard \
        --num_threads 16 data_${en_modeling_unit}/$x/wav.scp data/$x/text --segments data_${en_modeling_unit}/$x/segments \
        $(realpath data_${en_modeling_unit}/$x/shards) data_${en_modeling_unit}/$x/data.list
    else
      tools/make_raw_list.py data_${en_modeling_unit}/$x/wav.scp data/$x/text  --segments data_${en_modeling_unit}/$x/segments \
        data/$x/data.list
    fi
  done
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  torchrun --nnodes=$num_nodes --nproc_per_node=$num_gpus \
    ...
    wenet/bin/train.py \
      ...
      --cv_data data_${en_modeling_unit}/dev/data.list \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  python wenet/bin/recognize.py --gpu 0 \
    ...
    --data_type $data_type \
    --test_data data_${en_modeling_unit}/test/data.list \
