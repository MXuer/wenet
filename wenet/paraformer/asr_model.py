...
from wenet.paraformer.mae_loss import MAELoss
from wenet.paraformer.embeded_loss import EmbedLoss
...
class ParaModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.criterion_token_nums = MAELoss(normalize_length=length_normalized_loss)
        self.criterion_embed = EmbedLoss()
    ...
    def forward(
        ...
        loss_2pass: Optional[torch.Tensor] = None
        # 2a. Attention-decoder branch
        if self.ctc_weight != 1.0:
            loss_att, acc_att, loss_att_1st, loss_pred, loss_embed, sampler_info = self._calc_att_loss(encoder_out, encoder_mask, encoder_out_lens, 
                                                    text, text_lengths)
        else:
            loss_att = None
            loss_pred = None
        ...
        if loss_ctc is None:
            ...
            loss = loss_ctc
        elif loss_pred is not None and loss_embed is not None:
            loss = self.ctc_weight * loss_ctc + (1 -
                                                 self.ctc_weight) * loss_att + self.predictor_weight * loss_pred + self.embed_weight * loss_embed
        elif loss_pred is not None:
            loss = self.ctc_weight * loss_ctc + (1 -
                                                 self.ctc_weight) * loss_att + self.predictor_weight * loss_pred
        elif loss_embed is not None:
            loss = self.ctc_weight * loss_ctc + (1 -
                                                 self.ctc_weight) * loss_att + self.embed_weight * loss_embed
        else:
            loss = self.ctc_weight * loss_ctc + (1 -
                                                 self.ctc_weight) * loss_att
            
        if loss_att_1st is not None and loss_att is not None:
            loss_2pass = loss_att_1st - loss_att
        ...
        log_info = {"loss": loss, 
                    ...
                    "loss_2pass": loss_2pass}
        log_info.update(sampler_info)
        return log_info
    ...
    def _calc_att_loss(
    ...
    ) -> Tuple[torch.Tensor, float, torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, Optional[torch.Tensor]]]:
        ys_in_pad, ys_out_pad = add_sos_eos(ys_pad, self.sos, self.eos,
                                            self.ignore_id)
        ys_in_lens = ys_pad_lens + 1
        # get embeded acoustic featuers from encoder
        fired_frames, predicted_token_nums, r_fired_frames = self.predictor(encoder_out, encoder_out_lens, ys_out_pad)
        # reverse the seq, used for right to right decoder
        r_ys_pad = reverse_pad_list(ys_pad, ys_pad_lens, float(self.ignore_id))
        r_ys_in_pad, r_ys_out_pad = add_sos_eos(r_ys_pad, self.sos, self.eos,
                                                self.ignore_id)
        # Compute loss for token nums and predicted token nums
        target_token_num = (ys_out_pad != self.ignore_id).sum(-1)
        ...
        # Compute the loss between true label embedding and cif output
        loss_embed = torch.tensor(0.0)
        r_loss_embed = torch.tensor(0.0)
        if self.embed_weight > 0.0:
            loss_embed = self.criterion_embed(pred_acoustic_embeds, target_embed, tgt_mask)
            if self.reverse_weight > 0.0:
                r_loss_embed = self.criterion_embed(r_pred_acoustic_embeds, r_target_embed, tgt_mask)
                loss_embed = (1 - self.reverse_weight) * loss_embed + self.reverse_weight * r_loss_embed
        return loss_att, acc_att, loss_att_1st, loss_pred, loss_embed, sampler_info
    ...
    def recognize(
        ...
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        pred_acoustic_embeds, pred_token_nums, r_pred_acoustic_embeds = self.predictor(encoder_out, encoder_out_lens)

        pred_token_nums = pred_token_nums.long()
        decoder_out, r_decoder_out, _ = self.decoder(encoder_out, encoder_mask,
                                                     pred_acoustic_embeds, 
                                                     pred_token_nums, 
                                                     r_pred_acoustic_embeds,
                                                     self.reverse_weight)

        decoder_out = torch.nn.functional.log_softmax(decoder_out, dim=-1)
        decoder_out = torch.argmax(decoder_out, dim=-1)
        decoder_out = decoder_out.cpu().detach().numpy()
        r_decoder_out = torch.nn.functional.log_softmax(r_decoder_out, dim=-1)
        r_decoder_out = torch.argmax(r_decoder_out, dim=-1)
        r_decoder_out = r_decoder_out.cpu().detach().numpy()
        hyps = []
        for b in range(batch_size):
            current_hyp = []
            current = decoder_out[b, :]
            for ii, index in enumerate(current):
                if index == self.eos:
                    continue
                if ii == pred_token_nums[b]:
                    break
                current_hyp.append(index)
